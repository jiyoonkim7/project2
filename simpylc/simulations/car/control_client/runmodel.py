...
class NeuralNetwork(nn.Module): #from torch import nn
    def __init__(self):
        ...
        self.linear_1hiddenlayer = nn.Sequential(
            nnn.Linear(16, 64), #16 inputs to 1 output
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(64, 32),
            nn.SiLU(),
...
class HardcodedClient:
    def __init__ (self):
        ...
        with open (pm.sampleFileName, 'w') as self.sampleFile:
            with sc.socket (*sw.socketType) as self.clientSocket:
                ...
                while True:
                    self.input ()
                    self.sweep ()#Neuron Network, self infer
                    tm.sleep (0.1) #adjusting speed
                    self.output ()
                    self.logTraining ()
                    tm.sleep (0.02)
    ...
    def lidarSweep (self):
        ...
        lidardata = sample[0:16]
        print(lidardata)

        measurements_tensor = torch.tensor(lidardata).to(torch.double)
